...
def board_create_folder(domain_name):
    ...
    if(content_subject==data["board"]["folder_name_board_setting"]):
        ...
        #Logging(':'.join(["5. Add name folder =>pass",data["board"]["folder_name_board_setting"]]))
    else:
        Logging("5. Add name folder =>fail")
        ValidateFailResultAndSystem("<div>[Board]1. Input folder name </div>")
    Commands.Wait10s_InputElement(data["board"]["textbox_description_board"],data["board"]["description_board"])
    time.sleep(1)
